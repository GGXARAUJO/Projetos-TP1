...
void devolver_veiculo (Veiculos veic[], Clientes clie[], Locacao loca []) {
  ...
  int pv; //posição do veiculo na struct veiculo
    ...
    do {
      ...
      for (int i = 0; i < 50; i++) { //confere se o cliente está cadastrado
      ...
      for (int i = 0; i < 50; i++) {
        for (int j = 0; j < 50; j++) {
          if (clienteCadastrado) { //confere se o cliente tem uma locação
    ...
    dias_totais += dia_loca; // quantos dias tem do ano zero até o dia da locacao
    ...
    dias_locados += dia_devol; //quantos dias tem do ano zero ate a data da devolucao
    dias_locados = dias_locados - dias_totais; // quantidades de dias locados (devolucao - locacao)
...
int main () {
  struct Clientes clie[50]; // vetor de struct
  struct Veiculos veic[50]; // vetor de struct
...
/*Beatriz Queiroz Reis
  Gabriel Barbosa Baruque
  Guilherme de Gouvêa Araujo
  Rômulo Pastore da Cruz Silva*/